...
(function(factory) {
    ...
    var Utils = {
        ...
        sort: function(nodes, dir, width) {
            ...
            return _.sortBy(nodes, function(n) {
				var val = dir * (n.x + n.y * width);
				//n.el.find("header span").text("("+val+"= "+n.x+" + "+n.y+" * "+width+" )");
				return val; 
			});
        },
    ...
    GridStack.prototype._updateNodeWidths = function(oldWidth, newWidth) {
        ...
        for (var i = 0; i < this.grid.nodes.length; i++) {
			...
			node.el.find("header span").text("("+i+" )");
            this.update(
				node.el,
				Math.round(node.x * newWidth / oldWidth),		//X
				undefined,										//Y
				Math.round(node.width * newWidth / oldWidth),	//WIDTH
				undefined										//HEIGHT
			);
        }*/
		
		if (oldWidth > newWidth) {
			for (var i = this.grid.nodes.length-1; i >= 0; i--) {
				node = this.grid.nodes[i];
				node.el.find("header span").text("("+i+" )");
				if (node.x >= newWidth) {
					var newy = node.y+1;
					while (!this.isAreaEmpty(0, newy, node.width, node.height)) {
						newy++;
					}
					this.move(
						node.el,
    ...
    GridStack.prototype._updateNodePlaces = function(oldWidth, newWidth) {
        this.grid._sortNodes();
        this.grid.batchUpdate();
		...
		var sort = new HorizontalSort(newWidth);
		for (var i=0; i<nodes.length; i++) {
			var win = nodes[i];
			var free = sort.findFreePosition(win);
			console.log(i+".   X:"+free[1]+" - Y:"+free[0]);
			this.addWidget(win.el, free[1], free[0], win.width, win.height, false);
			sort.drop(win, free[0], free[1]);
		}
        this.grid.commit();
    };
    ...
    GridStack.prototype.setGridWidth = function(gridWidth,doNotPropagate) {
        ...
        if (doNotPropagate !== true) {
            this._updateNodePlaces(this.opts.width, gridWidth);
        }
...
function HorizontalSort(width) {
	
	var maxPositions = width+1; //1-4
	var pos = new Array();
	
	
	this.findFreePosition = function(win) {
		var i=0;
		var j=0;
		var sizes = getSizes(win);
		for (i=0; i<pos.length; i++) {
			var row = pos[i];
			for (j=0; j<maxPositions; j++) {
				if (row[j]!=1) {
					var enough = isPlaceEnough(i, j, sizes);
					if (enough) {
						return [i, j];
					}
				}
	...
	function isPlaceEnough(i, j, sizes) {
		for (var a=i; a<i+sizes[1]; a++) {
			var cell = pos[a];
			if (cell!=null) {
				for (var b=j; b<j+sizes[0]; b++) {
					if (b==maxPositions-1) return false;
					if (cell[b]==1) return false;
				}
			}
		}
		return true;
